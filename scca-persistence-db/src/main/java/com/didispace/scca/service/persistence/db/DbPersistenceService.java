...
public class DbPersistenceService implements PersistenceService {
	...
	public Properties readProperties(String application, String profile, String label) {
		...
		Properties properties=request(url,HttpMethod.GET,Properties.class,application, profile, label);
		return properties;
	}
	...
	public void deletePropertiesByEnv(String profile) {
		...
		Integer rows =request(url,HttpMethod.GET,Integer.class,profile);
	...
	public void deletePropertiesByProject(String application) {
		...
		for (Env env : envRepo.findAll()) {
			...
			Integer rows =request(url,HttpMethod.GET,Integer.class,application);
	...
	public void deletePropertiesByProjectAndEnvAndLabel(String application, String profile, String label) {
		...
		Integer rows =request(url,HttpMethod.GET,Integer.class,application, profile, label);
	...
	public void saveProperties(String application, String profile, String label, Properties update) {
		...
		Integer rows =request(url,HttpMethod.POST,Integer.class,application, profile, label);
	...
	public void updateProfileName(String oldName, String newName) {
		...
		Integer rows =request(url,HttpMethod.POST,Integer.class,oldName, newName);
		log.info("update profile name {} -> {}, change property rows {}", oldName, newName, rows);
	}

	private <T> T request(String url, HttpMethod method, Class<T> cls, Object... param) {
		...
		ResponseEntity<T> response = restTemplate.exchange(url, method, entity, cls, param);
